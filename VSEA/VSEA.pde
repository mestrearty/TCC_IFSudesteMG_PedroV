...
//Variação do Visualizador Sonoro de Entrada de Áudio em tempo real(VSAG),
...
AudioInput audio; //carrega e controla entrada de audio

//---------------------------------------

//Criando objetos de análise do audio (FFT)
FFT[] fft=new FFT[3]; //analisador do fast fourrier transform. 
//[0]-> mix(direito e esquerdo, [1]->esquerdo, [2]->Direito
//---------------------------------------
...
float escalaDoEspectro = 3; //O espectro é muito pequeno para se vizualisar,
//então ele é multiplicado por X vezes (essa variavel) para melhor compreenção 
...
void setup() {
  ...
  for (int i=0; i<3; i++) {
    fft[i] = new FFT( audio.bufferSize(), audio.sampleRate() );//cria o objeto fft, 
    //passando o tamanho do buffer do próprio audio (power of 2), 
    //lembrando que o tamanho da frequencia é sempre a metade dela em si (nyquist)
    fft[i].linAverages( 30 );//faz uma média e divide os valores 
    //da frequencia em um espaço de (X) (partição da frequência)
  }
...
void draw() {
  background(0);//fundo
  //carregando
  fft[0].forward(audio.mix);//analisa e constroi o fft de um audio. .mix para direita e esquerda,
  //.left esquerda, .right direita. Em caso de mais de uma variavel com fft, podemos utilizar para outras análises
  ...
  for (int j = 0; j < 3; j++) {//primeiro laço para percorrer os canais de áudio 1->mix, 2->3squerda, 3->direita
    ...
    {   
      ...
      if ( i+largura == mouseX  && j!=1)//compara a posição do mouse com a posição do spectro desenhado
      {
        ...
        stroke(cor);
        fill(255, 128);
        text("Média da frequência do spectro: " + centroDaFrequencia, largura+100, height2 - 100);
        //exibe a frequencia do ponto específico pegado anteriormente
      } else
      {
        if (largura-i == mouseX  && j==1) {
          stroke(cor);
          fill(255, 128);
          text("Média da frequência do spectro: " + centroDaFrequencia, width*1/3+100, height2 - 100);
          //exibe a frequencia do ponto específico pegado anteriormente
        } else {
          stroke(255);//volta a coloração branca
        }
      }
      if (j==0 || j==2)line(i+largura, height2, i+largura, height2 - fft[j].getBand(i)*escalaDoEspectro); 
      //primeiro campo é o eixo x, no caso i, que é qual parte do spectro , depois a posição y, x, depois inverte Y * escala do spctro
      if (j==1)line(largura-i, height2, largura-i, height2 - fft[j].getBand(i)*escalaDoEspectro); 
      //primeiro campo é o eixo x, no caso i, que é qual parte do spectro , depois a posição y, x, depois inverte Y * escala do spctro
    }
    //--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

    //início do desenho onde definimos o tamanho da partição da frequencia, no caso 30
    if (j==1)largura-=w;//por conta do espelhamento do lado esquerdo,
    //para evitar que haja interceção entre os primeiros plots, recuo uma casa (w)
    for (int i = 0; i < fft[0].avgSize(); i++)//percorre a partição
    {
      ...
      if ( mouseX >= i*w+largura && mouseX < i*w + w+largura  && j!=1)
      {
        ...
        fill(255, 128);
        text("Média Linear da frequência do spectro: " + centroDaFrequencia, largura+100, height - 100);
        //exibe a frequencia do ponto específico pegado anteriormente
      ...
      } else
      {
        if ( mouseX >= largura-i*w && mouseX <  w+largura - i*w && j==1)
        {
          ...
          fill(255, 128);
          text("Média Linear da frequência do spectro: " + centroDaFrequencia, width*1/3+100, height - 100);
          //exibe a frequencia do ponto específico pegado anteriormente
      ...
      noStroke();
      if (j==0 || j==2)rect(i*w+largura, height, i*w + w+largura, height - fft[j].getAvg(i)*escalaDoEspectro);
      //desenha igual ao anterior, só que respeitando a partição
